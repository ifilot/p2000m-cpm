...
KERNEL_API = ('return_zero', 'return_ff', 'return_a', 'selected', 'dph_a',
              ...
              'bdos_dpb', 'bdos_user', 'bdos_reset_drives', 'bdos_disk_error',
              'fs_entry_extent', 'fs_position', 'fs_sync_fcb', 'fs_validate_block')
...
def link_core(root, output):
    ...
    names = []
    for source in ('filesystem.asm', 'cache.asm', 'console.asm', 'keyboard.asm', 'keyboard_repeat.asm', 'disk_io.asm', 'rom_tables.asm', 'sd_crc.asm'):
    ...
    contract = {'version': (root / 'VERSION').read_text().strip(),
                'ram': {key: kernel[key] for key in KERNEL_API},
    ...
    assert rom['rom_crc_end'] <= LAYOUT['rom_end']
    ...
    return {**LAYOUT, **{key: kernel[key] for key in ('bios', 'bdos_entry', 'resident_code_end', 'cold_code_end',
            ...
            **{key: rom[key] for key in ('rom_driver_end', 'rom_runtime_end', 'rom_filesystem_end', 'rom_keyboard_tail_end', 'boot_loader_end',
                                        ...
                                        'key_overflow', 'key_repeat_count', 'rom_crc_end',
                                        'spi_tx', 'cmd_send', 'cmd_crc', 'sd_crc16_check',
                                        'read_byte', 'write_byte', 'write_crc', 'sd_read_retry',
                                        'sd_crc7_byte', 'sd_crc16_byte', 'sd_crc_fail',
                                        'boot_cid_byte', 'boot_cid_read', 'boot_recovery', 'loader_recovery')}}
